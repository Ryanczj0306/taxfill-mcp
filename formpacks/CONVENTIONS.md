...
- `<form_key>` MUST be one of:
  ...
  `f8843`, `f8863`, `f2555`, `f1040nr`, `f1040`, `sched_1`, `sched_2`,
  `sched_3`, `sched_a`, `sched_b`, `sched_c`, `sched_d`, `sched_e`,
  `sched_oi`, `sched_se`
...
```
uv run --no-sync pytest packages/core/tests/test_formpacks_federal.py -q -m "not network" -k "<tax_year>-<form_key>"
```
...
```
uv run --no-sync pytest packages/core/tests/test_formpacks_federal.py -q -k "<tax_year>-<form_key>"
```
