...
3. For all 35 adopted 2023 jurisdictions — the eight flat-rate states (IL, PA, IN, MI, NC, CO, KY, AZ) and the 27 graduated-bracket ones (AL, AR, CA, DC, DE, GA, ID, KS, LA, MD, ME, MN, MO, MS, MT, ND, NE, NJ, NY, OH, OK, OR, RI, VA, VT, WI, WV) — compute the state tax line with `calc("state_tax", {state, taxable_base, exemptions_count, dependents_count, filing_status, year})` and pass it into verify's `independent` (e.g. `independent={"12": <calc state_tax>}` for IL-1040 Line 12) — the state tax line is never your own arithmetic. Supply the STATE's own base (IL: Line 9 base income; CO: fed taxable income ± mods; PA: the eight-class PA-source income; graduated packs name the exact line in their `tax_line`/notes) and heed the work string's disclosures: tax-table mandates (the filed form takes the booklet table, which can differ by a few dollars within a band), schedule quirks that bracket math cannot reproduce (NY's benefit-recapture worksheets above $107,650 NYAGI; OH's $360.69 schedule jump at $26,050; AR's bracket-adjustment phase-down above $89,600), credit-style exemptions the op refuses counts for (CA/OR/NE/AR $29-$446 per-person credits), county/city add-ons (MD's mandatory county rate, OH school-district tax), and state-specific status mappings (KS files federal QSS as head of household; AL/WV have no QSS analog).
