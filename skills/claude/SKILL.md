...
| `hand_fill_worksheet(form, year, jurisdiction, values?)` | print-and-copy worksheet for the 4 print-only (non-AcroForm) states: **CT, HI, NM, SC**. These packs are NOT returned by `list_forms` — if `list_forms("states/ct"\|"states/hi"\|"states/nm"\|"states/sc", …)` comes back empty, that is expected: go straight to `hand_fill_worksheet` (`ct1040` / `n11` / `pit1` / `sc1040`). **Also the FBAR**: FinCEN Form 114 is e-file only through FinCEN's BSA E-Filing System and irs.gov states a PRINTED Form 114 is not accepted, so it has no fillable pack either — `hand_fill_worksheet('fincen114', <year>, 'federal')` gathers the values (and computes the $10,000 aggregate-maximum test) for keying into the BSA system. It is filed with FinCEN, NOT the IRS, and is NOT part of the return envelope. |
| `calc(op, args)` | tax / standard_deduction / se_tax / additional_medicare_tax / niit / tax_with_preferential_rates / taxable_social_security / excess_ss / student_loan_interest_deduction / education_credits / ptc_annual / ptc_monthly (Form 8962 lines 12-23 grid — part-year 1095-A coverage takes 12 `{premium, slcsp, aptc}` rows) / child_tax_credit / eitc / dependent_care_credit (Form 2441 → Schedule 3 line 2: `{expenses, qualifying_persons, earned_income, spouse_earned_income?, agi, filing_status, year, employer_benefits?}` — spouse_earned_income REQUIRED for MFJ, W-2 box 10 benefits reduce it, MFS gets $0 by rule, 2021 is refundable only with a US abode) / treaty_benefit (validate a treaty article + dollar limit from the per-country packs — china/india/korea/canada/mexico; `income_class` in student_wages\|scholarship\|payments_from_abroad\|teacher_wages\|other_income, `years_in_status` for teachers; pass `resident_alien` — WHERE the exempt amount is reported turns on residency (P-016): a nonresident uses Schedule OI item L and the 1040-NR treaty-exempt line, a RESIDENT alien claiming a saving-clause exception enters it in parentheses on Schedule 1's other-income line with "Exempt income", the country and the article (Pub 519 ch. 9), paired with the income on its usual line when an information return reported it as taxable (income left off the return is not also subtracted); when the applied article has NO saving-clause exception in the pack (Canada Art. XV's de-minimis) the work says a resident generally cannot claim it; the work names the year's line, never assume one. `other_income` answers the bank-bonus / 1099-MISC box 3 corner from the verbatim Other Income article: in ALL five treaties US-arising other income stays US-taxable — Schedule NEC at the statutory 30%, no treaty reduction; Korea verifiably has NO other-income article) / schedule_1a_deductions (the four OBBBA Schedule 1-A deductions, TY2025-2028: `{magi, filing_status?, year?, qualified_tips?, qualified_overtime?, car_loan_interest?, seniors_qualifying?}` — line 38 → 1040 line 13b / 1040-NR line 13c. TRAPS enforced: tips/overtime/senior FORFEITED on MFS (car loan is not); tips $25,000 cap is PER RETURN (MFJ does not double it); asymmetric rounding ($100/$1,000 of excess MAGI rounded DOWN for tips/overtime; $200/$1,000 rounded UP for car loan; 6% of excess per person for senior); QSS takes the non-joint thresholds; overtime input is the FLSA premium HALF only. Occupation-list / new-vehicle-VIN / SSN eligibility stays YOUR judgment, quoted in the work) / employee_fica (employee payroll FICA across STATUS segments `[{wages, fica_exempt, label?, visa_status?, exempt_basis?}]` plus `residency_classification?` — the projection op for the F-1-OPT→H-1B year; fica_exempt is YOUR per-segment status judgment, and the F/J exemption is STATUS-based, not marital: §6013(g) does NOT start FICA on an exempt F/J spouse's wages. It is also a NONRESIDENT exemption: Pub 519 ch. 8 — FICA is withheld "if you are considered a resident alien … even though your nonimmigrant classification (“F,” “J,” “M,” or “Q”) remains the same", so an F-1 student who has become a resident owes it; pass the residency tool's classification and a 'resident' F/J/M/Q segment marked exempt is REFUSED unless `exempt_basis` is `student_employed_by_school` or `totalization_agreement`. SS wage base applied across segments; 0.9% Additional Medicare over $200k) / estimated_tax_safe_harbor (IRC 6654(d): min(90% current-year tax, 100%/110% prior-year — 110% when PRIOR AGI > $150k/$75k-MFS keyed on the CURRENT year's status); pass prior_year_agi AND prior_year_total_tax together (prior 1040 lines 11/24) or neither; $1,000 de-minimis + quarterly shortfall; the work quotes the bonus-withholding trap WITH its precondition (P-017): the flat 22% is the employer's OPTION only under Treas. Reg. 31.3402(g)-1(a)(7)(i) — the bonus not paid concurrently with regular wages or separately stated on the payroll records, AND income tax withheld from regular wages this calendar year or last — otherwise the AGGREGATE procedure is required (Pub 15 "method 1b": bonus + regular wages withheld as one payment under the W-4); 37% is mandatory only over $1,000,000 of supplemental wages. Hire status is not the test — take the bonus withholding from the pay stub, never assume 22% × bonus) / annualize_ytd (YTD paystub → full-year by calendar-day proration; LEVEL-PAY assumption — annualize each status segment separately, never one-time amounts) / contribution_limits (`{year?}` — every tax-advantaged bucket WITH its scoping: 402(g) per PERSON all-employers traditional+Roth shared; 415(c) per EMPLOYER PLAN; IRA per person both kinds; HSA per COVERAGE TIER — two self-only holders can beat one family plan; FSA per employee per employer; commuter monthly) / ira_contribution_eligibility (`{magi, filing_status?, year?, ira_type?, contributed?, ...}` — the Pub 590-A worksheet (round UP to $10, $200 floor). Run BEFORE recording any IRA contribution: it catches the 6%/yr-excise excess error and shows the year-end-status rule (same MAGI: excess under one status, compliant under another, e.g. MFS vs MFJ — eligibility follows the Dec 31 filing status); the traditional_deduction path needs employer-plan coverage facts) / marginal_dollar_savings (`{taxable_income, wages, filing_status?, year?}` — payroll HSA/FSA/commuter dollars avoid income tax AND FICA, 401(k)/IRA dollars income tax only; above the SS wage base the FICA saving is 1.45%+0.9% over $200k, never 7.65%) / magi_ladder (`{agi, filing_status?, year?, wages?, ...}` — every MAGI test the year's packs carry in one table; show it when the user asks why MAGI differs from salary — Additional Medicare is a WAGE test, NIIT adds back the FEIE, each IRA test defines its own) / ira_pro_rata (`{dec31_total_value, amount_converted?, other_distributions?, nondeductible_basis_carryforward?, nondeductible_contributions_this_year?, contributions_made_after_year_end?, year?}` — Form 8606 Part I / IRC 408(d)(2): ALL traditional + SEP + SIMPLE IRAs are **one contract** and a year's distributions **one distribution**, so a conversion's taxable share comes from the POOL's pretax/basis mix, never from which dollars moved. THE TRAP: the DENOMINATOR is line 9 = line 6 (Dec-31 value of every such IRA, measured AFTER the conversion left it) + line 7 (distributions not converted) + line 8 (the amount converted) — it **adds the conversion back**, so converting more never shrinks it. `dec31_total_value` is the whole answer to "where can my old 401(k) go?": rolled into a traditional IRA it lands on line 6 whenever in the year it arrives and makes every future backdoor Roth mostly taxable. Basis is per PERSON (the form is filed separately per spouse, so a spouse's IRA never enters your ratio), and line 4 — a contribution made Jan 1-Apr 15 of the NEXT year — is basis that carries forward but cannot be used in this year's ratio. Its taxable figure — not 1099-R box 2a, which for a traditional IRA or a Roth conversion is the GROSS amount with 2b "Taxable amount not determined" checked (i1099r) — is what `estimate_refund`'s `retirement_income_taxable` takes; box 7 codes N/R and H are $0 taxable, and code G is $0 except a Roth-bound direct rollover from a pre-tax plan (to a Roth IRA, or an in-plan Roth rollover) and designated Roth employer contributions, where box 2a is the taxable amount) / roth_conversion (`{source, amount, taxable_income_before, magi_before, filing_status?, year?, net_investment_income?, plan_after_tax_basis?, dec31_total_value?, ...}` — `source` is the whole point, because the two paths are taxed differently and get conflated. `plan_to_roth_ira` = a DIRECT rollover from a 401(k)/403(b)/governmental 457(b) to a Roth IRA (Notice 2008-30): taxable is the pretax portion, **pro-rata never applies** because no traditional IRA is involved, and it is reported on 1040 line 5a/5b, not Form 8606 — this is the path that clears an old plan WITHOUT poisoning future backdoor Roths; `plan_after_tax_basis` comes from 1099-R box 5 (the plan's own allocation under Notice 2014-54, never your estimate). `traditional_ira_to_roth` = pro-rata applies, delegated to ira_pro_rata, so it needs `dec31_total_value`. Passing IRA args on the plan path is REFUSED — that conflation is why the op makes you choose. It also returns the two things no filer computes by hand: BRACKET HEADROOM (room below the current bracket's top and exactly which dollars spill into higher rates, from the year's rate_schedules) and the NIIT CROSSING (IRC 1411(c)(5): the conversion is **never** net investment income, but it raises MAGI and can drag OTHER investment income over the §1411 threshold). Quote the work's WITHHOLDING warning — tax withheld is not converted, it is lost Roth space and the 10% additional tax can reach it under 59½, so pay from OUTSIDE funds — plus the no-recharacterization-after-2017 irreversibility and the per-conversion 5-year clock) / hsa_deduction (`{coverage: self_only|family, year?, months_eligible?, monthly_coverage?, age_55_plus?, married?, spouse_has_separate_hsa?, personal_contributions?, employer_contributions?, medicare_start_month?, health_fsa?, distributions_total?, qualified_medical_expenses?, testing_period_failed?, wages?, …}` — Form 8889 / IRC 223, the op that makes an HSA contribution FILEABLE. FOUR TRAPS. **The limit is MONTHLY**: 223(b)(1)-(2) tests eligibility on the FIRST DAY of each month and gives 1/12 of the tier amount per eligible month, so a mid-year HDHP start takes the Line 3 Limitation Chart, never the annual figure — pass `monthly_coverage` (12 entries, January first, each `self_only`/`family`/`none`) whenever the tier changed or the eligible months are not the closing ones. **The last-month rule** (223(b)(8)) is automatic, not an election: eligible on Dec 1 makes line 3 the GREATER of the chart and December's full annual tier, and it starts a **13-month testing period** (Dec 1 → Dec 31 of the FOLLOWING year); lose eligibility inside it — new job, spouse's non-HDHP plan, Medicare, anything but death or disability — and the extra comes back as income PLUS a 10% additional tax. Quote `at_risk_if_testing_period_fails` BEFORE anyone contributes the bigger number. **W-2 box 12 code W is not a second deduction**: it is the employer's money AND the employee's cafeteria-plan payroll deferrals, already out of box 1, so it belongs in `employer_contributions` (line 9) where it SUBTRACTS room — only direct contributions (line 2 = 5498-SA box 2 − code W − any funding distribution) reach Schedule 1 Part II line 13. **A general-purpose health FSA disqualifies you — including one from the SPOUSE's employer** (Rev. Rul. 2004-45 Situation 1 holding); `health_fsa='general_purpose'` is refused with the fix, `limited_purpose`/`post_deductible` are fine. Also modelled: 223(b)(7) zeroes every month from the first month of Medicare entitlement (retroactive enrollment included); the $1,000 age-55 catch-up is statutory, PER PERSON and NOT allocable, so a couple needs TWO HSAs for two catch-ups and it rides line 7 rather than line 3 once a married filer has ANY family coverage; IRC 4973's 6% excise runs EVERY year until the excess plus earnings comes out by the due date INCLUDING extensions; Part II distributions (1099-SA box 1) with the 223(f)(4) 20% tax and Part III recapture. Pass `wages` for the payroll-vs-direct FICA half — above the SS wage base that saving is 1.45%, or 2.35% over $200k, never 7.65%) / espp_disposition (`{shares, grant_date, purchase_date, sale_date, grant_date_fmv_per_share, purchase_date_fmv_per_share, purchase_price_per_share, sale_price_per_share, grant_date_exercise_price_per_share?, broker_reported_basis?, broker_reported_basis_to_irs?, selling_expenses?}` — ONE lot of §423 ESPP stock, Form 3922 boxes 1-8 in and the Form 8949 row out. **The two dispositions are taxed completely differently.** **QUALIFYING** (sold MORE than 2 years after GRANT *and* more than 1 year after PURCHASE, IRC 423(a)(1)): ordinary income is the **LESSER** of the discount on the **GRANT-date** price (423(c)(2)) and the actual gain (423(c)(1)) — so a sale at a LOSS recognises **zero** ordinary income (Pub 525: "If you have a loss from the sale, it's a capital loss, and you don't have any ordinary income"). **DISQUALIFYING** (anything else, same-day sale included): ordinary income is the **full spread at PURCHASE** (exercise-date FMV − price paid) **regardless of the sale price** — "This ordinary income isn't limited to your gain from the sale of the stock" — so someone who sold below the purchase-date FMV still recognises it *and* takes a capital loss. **THE BASIS CORRECTION IS THE HIGHEST-DOLLAR PART:** the 1099-B box 1e is the DISCOUNTED PURCHASE PRICE only (Instructions for Form 8949: for compensatory options granted after 2013 the reported basis "won't reflect any amount you included in income upon grant or exercise"), so trusting it taxes the discount **twice**. The op returns the corrected basis (paid + ordinary income), the adjustment and the exact Form 8949 row: **code B** in column (f), wrong basis left in (e) with the correction in (g) as a negative when basis WAS reported to the IRS (box A/D), correct basis in (e) with -0- in (g) when it was not (box B/E). **THE LOOKBACK:** the price paid is the discount off the LOWER of the two FMVs, but 423(c)(2) is measured on the GRANT-date price — **Form 3922 box 8** — a different number whenever the stock fell between grant and purchase; a price below 85% of the grant-date FMV with no box 8 is REFUSED, because only a lookback can produce it. **Nothing is withheld** on any of it (IRC 423(c), 421(b); IRC 3121(a)(22) keeps it out of FICA) — plan the cash. ISO/AMT, §83(b), RSU vesting and wash sales are NOT modelled and are named in `not_modeled`) / capital_loss_limitation (`{short_term, long_term, taxable_income_before_capital_loss, filing_status?, year?, short_term_carryover_in?, long_term_carryover_in?, following_years?}` — IRC 1211(b)/1212(b) and Schedule D's Capital Loss Carryover Worksheet. Short and long net **separately** first (lines 7 and 15) and only then against each other (line 16); a net loss deducts at most **$3,000 ($1,500 MFS)** — statutory, never indexed — and the rest carries forward **indefinitely with its character preserved**: a short-term carryover stays short-term (1212(b)(1)(A)) and goes on next year's line 6, long-term on line 14. **The subtlety is not the one people expect:** taxable income does NOT shrink the $3,000 deduction, it shrinks how much of the LOSS POOL the year consumes — worksheet line 4 = min(deduction, taxable income + deduction) is 1212(b)(2)(A)'s lesser-of, so $1,000 of taxable income deducts $3,000 and burns only $1,000, leaving a carryover **$2,000 larger** than "loss − $3,000". Read `loss_absorbed` and `deduction_not_absorbed`, not just `deduction`. Pass `taxable_income_before_capital_loss` WITHOUT the line 21 deduction taken out (negative is fine — the worksheet wants it); the op derives worksheet line 1 and prints it to tie against Form 1040 line 15. `following_years` rolls the chain forward and **threads the carryovers itself** — re-entering them by hand is how the character split gets lost, so unknown keys are refused) / foreign_tax_credit_election (`{creditable_foreign_taxes, all_foreign_income_passive, all_reported_on_payee_statement, year?, filing_status?, elect?, entity?, regular_tax?, reduction_in_foreign_taxes?, excess_credit_if_form_1116?}` — **IRC 904(j): the branch that decides whether Form 1116 is needed at all**, and for anyone holding a total-international index fund the answer is usually NO. Foreign tax withheld on fund dividends shows up in **Form 1099-DIV box 7** (interest: 1099-INT box 6), and while creditable foreign taxes stay at or under **$300 ($600 in the case of a JOINT RETURN** — 904(j)(2)(B), statutory and never inflation-indexed) the whole credit is ONE NUMBER on **Schedule 3 (Form 1040), Part I, line 1** with no Form 1116, no three-country column grid and no 904(a) limitation fraction. Run it BEFORE you start filling f1116. **THREE THINGS IT REFUSES TO GUESS**, because each silently decides the election: `all_foreign_income_passive` — 904(j)(2)(A) needs the **ENTIRE** foreign-source gross income to be qualified passive income, so one dollar of foreign wages, foreign self-employment income, foreign rent or a §951A inclusion kills it for the WHOLE YEAR however small the tax; `all_reported_on_payee_statement` — 904(j)(3)(A)/(B) need the income **and** the tax shown on a payee statement (1099-DIV, 1099-INT, Schedule K-1 (1041), Schedule K-3 (1065), Schedule K-3 (1120-S) or a substitute), so foreign tax on a foreign bank account that sends no US information return fails it; and `elect` — 904(j)(2)(C) is the taxpayer's own act, so `election_available` and `election_made` come back SEPARATELY and a filer who qualifies may still prefer Form 1116. **Estates and trusts are excluded outright** (904(j)(3)(D)). **ALWAYS QUOTE WHAT IT COSTS — it is not free:** 904(j)(1)(B) and (C) forfeit the 904(c) 1-year-back / 10-year-forward carryover in **both directions** for the election year, so pass `regular_tax` (Form 1116 line 20 = Form 1040 line 16 + Schedule 2 line 2, less any Form 4972 tax) and read `credit_lost_to_regular_tax_cap` — foreign tax above regular tax is claimed by nobody, ever. The election does NOT waive creditability (IRC 901/903, including **901(k)'s minimum holding period** on dividend withholding) nor the Form 1116 line 12 reduction, which `reduction_in_foreign_taxes` subtracts BEFORE the $300/$600 test. **ONE TRAP THE FILING-STATUS COLUMN HIDES:** only married filing jointly is "a joint return", so a **qualifying surviving spouse gets $300, not $600** — it borrows the joint RATE schedule (IRC 1(a)(2)/2(a)) but does not file a joint return; MFS is $300 too. When the election is unavailable, file `formpacks/federal/<year>/f1116` — one form PER CATEGORY OF INCOME, exactly one of boxes a-g ticked above Part I, and box **c** (passive) is the 1099-DIV box 7 basket) / foreign_asset_reporting (**DOES THIS FILER HAVE TO REPORT A FOREIGN ACCOUNT?** — Form 8938 (IRC 6038D) and the FBAR (31 U.S.C. 5314) answered TOGETHER, because answering one and stopping is the standard error: "The Form 8938 filing requirement does not replace or otherwise affect a taxpayer's obligation to file FinCEN Form 114". `{year?, filing_status?, us_person?, lives_abroad?, specified_asset_value_year_end?, specified_asset_value_max?, foreign_account_value_max_aggregate?, has_foreign_account_signature_authority?, filer_type?}`. NOT a tax calculation — **both duties exist even when the account earned nothing and no tax is owed** (Treas. Reg. 1.6038D-2(a)(8)), which is exactly why a refund-shaped interview never surfaces them: **ASK** (intake question `income_documents.foreign_accounts`), do not wait to be told. THE TWO THRESHOLDS ARE NOTHING ALIKE. **Form 8938**, over the TAXABLE year: more than $50,000 on the LAST DAY *or* more than $75,000 AT ANY TIME (either alone triggers it, so emptying the account before Dec 31 does not help), $100,000/$150,000 married filing JOINTLY, and 4x — $200,000/$300,000 and $400,000/$600,000 — only for an **IRC 911(d)(1) qualified individual** (bona fide foreign residence for a full tax year, or 330 full days abroad in 12 consecutive months, with a foreign tax home); living overseas without meeting that test keeps the in-US figures. **A qualifying surviving spouse takes $50,000/$75,000, not the joint figures** — Treas. Reg. 1.6038D-2(a)(2)/(a)(4) are keyed to filers who "file a joint annual return" and a QSS does not, even though it borrows the joint rate schedule. **FBAR**, over the CALENDAR year: a flat **$10,000 that filing status does not move**, and it is an AGGREGATE across every foreign financial account and a MAXIMUM-VALUE test — two accounts whose combined balance touched $10,001 for one day are BOTH reportable even if each stayed under $10,000 and both were empty at year end. **The op refuses to guess**: any fact it needs and does not have comes back as `required=None` with a `must_ask` question naming the authority, so read `must_ask` / `any_duty_undecided` before quoting a verdict. Where they go: Form 8938 attaches to the return (`f8938`); the FBAR is e-filed with FinCEN via `hand_fill_worksheet('fincen114', …)`, due April 15 with an AUTOMATIC extension to October 15 needing no request (P.L. 114-41 §2006(b)(11) — 31 CFR 1010.306(c)'s printed "June 30" was never conformed to the statute and is WRONG). Scopes differ BOTH ways: signature authority over someone else's account and an account at a foreign BRANCH of a US bank are FBAR-only; foreign stock outside an account, a foreign partnership interest and a foreign hedge fund are 8938-only. Penalties quoted in the result: IRC 6038D(d) $10,000 + $10,000 per 30 days after IRS notice to a $60,000 year cap, and 6662(j)(3) 40% on an underpayment tied to an undisclosed foreign asset; 31 U.S.C. 5321(a)(5) non-willful up to $10,000 — adjusted to $16,536 for penalties assessed on or after 2025-01-17 (31 CFR 1010.821) and accruing **per REPORT, not per account** (Bittner v. United States, 598 U.S. 85 (2023)) — willful the greater of $100,000 (adjusted $165,353) or 50% of the balance, which IS per account) / state_tax (the STATE income-tax line for **ALL 42 income-tax jurisdictions**, tax years **2023, 2024 and 2025** (42/42 each): `{state, taxable_base, exemptions_count?, dependents_count?, filing_status?, year}`. The PACK decides the shape — flat-rate states multiply, graduated states apply the per-filing-status marginal schedule bracket by bracket with every bracket's contribution in the work string. **Never assume which shape a state is, and never carry a roster forward: the split moves by year** (GA converted to flat for 2024; IA and LA for 2025). `taxable_base` is the CALLER's job — supply the STATE's OWN base already adjusted for that state's additions/subtractions (IL: IL-1040 Line 9 base income = fed AGI ± IL mods; CO: fed TAXABLE income ± mods; PA: the eight-class PA-source income, no exemptions/deductions; every other pack names its exact line in `tax_line`/notes). The op applies only the pack's verified personal/dependent exemptions + standard deduction and errors prescriptively otherwise — credit-style exemption states (CA/OR/NE/AR) **refuse counts**, because their per-person amounts are CREDITS, not exemptions. Heed the work string's disclosures: tax-table mandates below a threshold (the filed form takes the booklet table, which can differ by a few dollars within a band), out-of-schedule surcharges/recaptures (CA's 1% Mental Health Services Tax over $1M; NY's recapture worksheets above $107,650 NYAGI, where plain bracket math is **WRONG**), OH's $360.69 schedule jump at $26,050, AR's bracket-adjustment phase-down above $89,600, KS's zero-tax cliff + QSS→HoH mapping, and county/city add-ons (MD's mandatory county rate, OH school-district tax) — NOT modeled) — every result shows its work + citation |
...
> are CLOSED.** As of the 2026-08 backfill the federal set is **111 packs** and
...
2. Required forms: `f1040nr` + `f8843`, plus `sched_oi` (treaty), `sched_a_nr` (itemized deductions — nonresidents cannot take the standard deduction except Indian students under Art. 21(2)), `sched_nec` (FDAP income at flat/treaty rates), `sched_1`, `sched_c` if self-employed. Treaty positions (e.g. US-China Art. 20(c) on student-period wages) are decided with the user and recorded (pass the confirmed amount as `treaty_exempt_income` in the estimate); eligibility is per visa **period**. A treaty claim that SURVIVES into a resident year (a saving-clause exception) is not a Schedule OI item: the resident's Form 1040 carries it in parentheses on Schedule 1's other-income line (Pub 519 ch. 9) — `treaty_benefit` with `resident_alien: true` and the estimate's assumption name the year's line. US bank-deposit interest is excluded for a nonresident (IRC 871(i)(2)(A)) only when its character is confirmed — pass that subset of `interest` as `bank_deposit_interest`.
...
but only while NONRESIDENT: once the filer is a resident alien, Pub 519 ch. 8 has it
withheld "even though your nonimmigrant classification … remains the same", so FICA
on a resident year's wages is NOT withheld in error ON THE F/J/M/Q GROUND. A
different exemption can still cover a resident's wages — the
student employed by the school it attends (Pub 519 ch. 8 tip: "Any student who is
enrolled and regularly attending classes at a school may be exempt from social
security and Medicare taxes on pay for services performed for that school") or a
totalization agreement — and FICA withheld despite one of those is recovered the same
way, from the employer first, then on Form 843 citing that exemption (Pub 519 ch. 8,
"Refund of Taxes Withheld in Error"). Employers commonly withhold it
from exempt nonresidents anyway — W-2 boxes 4/6 nonzero on an exempt nonresident
means withheld in error (intake's FICA note flags it; `estimate_refund`
...
`hand_fill_worksheet`. `state_scope` drives the list. (`hand_fill_worksheet` also
