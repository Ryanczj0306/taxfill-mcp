...
A running log of gaps surfaced by driving an agent end to end through **hypothetical demo cases**, as opposed to the
demo cases**, as opposed to the static persona reviews that produced Phase F/G. Each
the demo case posed, what the engine/intake did with it, and the smallest honest fix.
...
- **No per-period FICA attribute.** The single biggest cash-flow fact of the transition year —
...
- **The H-1B start date is easily under-specified.** Filers routinely give the offer
...
- A **household** budget means two returns, one rent, one cash-flow question.
...
- If a couple marries mid-year, the §6013(g)/(h) election (already built, Tier 2) becomes a
...
[`INTAKE_WORKSHEET.md`](INTAKE_WORKSHEET.md) (localized copies alongside) — a fill-in worksheet whose three
opening rules are *"don't guess, write 'don't know'"*, *"every identity/address fact is a date
...
**Follow-up (a filing-posture what-if).** A household considering marriage needs a
marry-vs-stay-single comparison. Modelling one exposes four more gaps, all of which have to be
computed OUTSIDE the engine without them — each can change the answer by
(text revised)
...
- **N-6 — Schedule 1-A (OBBBA) is not modeled at all.** The 2026 return carries four new
  ...
  *explicitly attaches to Form 1040-NR too* (line 38 → "Form 1040-NR, line 13c"), plus the
  ...
  charity. Two of its mechanisms bear directly on a marry-vs-single what-if: the overtime
  ...
  phase-out can move when a couple starts filing jointly. Needed: a
...
### The coverage verdict (the most useful finding)

Of the **22 MCP tools**, such a planning drive reaches only **four** — `intake_checklist`, `state_scope`,
(text revised)
(text revised)
Every number that actually **decides the answer** has to be produced outside the engine, in
(text revised)

| What decides such an answer | Where it had to come from |
|---|---|
| Schedule 1-A phase-outs (tips, overtime, car-loan interest, senior) | hand-coded from the form PDF |
(text revised)
(text revised)
(text revised)
(text revised)
(text revised)
| Excess Roth IRA contribution detection | left to the agent, not the tool |

(text revised)
is exactly what does **not** happen: such a drive rediscovers a large pile of citable 2026
(text revised)
(text revised)
...
- **N-10 — no knowledge of tax-advantaged account limits, and none of their SCOPING.**
  common question is *"is the 401(k) limit one per person?"* — and the interesting
  ...
  coverage tier** (so two people, each with self-only HDHP coverage, get $4,400 × 2 = $8,800,
