...
- Eval scenarios v1: (a) F-1 back-filing w/ treaty + 1099-NEC, (b) simple W-2 federal+CA resident, (c) part-year CA nonresident with remote work, (d) refund + direct deposit, (e) balance due paid online vs by check, (f) no-income-tax state ("nothing to file" answer), (g) **F-1 → H1B mid-year transition claiming Art. 20(c) on student-period wages**, (h) user who moved after the tax year (current vs historical address handling), (i) **post-2025 law change** (e.g., OBBBA car-loan-interest deduction on a 2026 filing): agent must resolve caps/eligibility via `get_sources` and cite IRS guidance — hallucinated numbers fail the eval, (j) **estimate accuracy & honesty**: for the simple W-2 scenario, the early estimate from W-2-only input must bracket the final computed refund and the range must tighten as intake completes; an estimate presented without its assumption list, or a point value presented as exact, fails the eval, (k) **married filing jointly**: two W-2s (taxpayer + spouse) on one return — correct MFJ standard deduction/brackets, spouse name/SSN filled, both-signature checklist, (l) **MFJ vs MFS comparison**: the server computes the return both ways and recommends the lower-tax option with the dollar delta shown; recommending without showing both, or ignoring the joint-liability caveat, fails, (m) **NRA-spouse §6013(g) election**: an F-1/H-1B couple with one nonresident-alien spouse elects to file MFJ — the worldwide-income trade-off and the election statement are surfaced and cited; silently filing MFJ for an NRA spouse without the election fails.
