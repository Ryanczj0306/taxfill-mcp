...
LLM agents can already do high-quality tax prep when a skilled operator drives them and checks every step. What blocks normal users:
...
5. **Positions.** Agent + user decide **filing status** (incl. married-filing-jointly vs married-filing-separately — computed both ways and compared when it matters, plus any §6013(g)/(h) election for an NRA spouse), elections/treaty articles/credits; every decision and its authority is recorded in a generated `RECONCILIATION.md`.
...
8. **Summary (bottom line first).** Before anything is printed: per-jurisdiction plain-language summary — *"Federal 2025: refund $1,250, direct-deposited to …1234. Federal 2024: you owe $380 (+ late penalties the IRS will bill separately, roughly $X–Y). California: no return required because …"* — with deadline status (including the 3-year refund statute of limitations). User approves the bottom line before the print step.
...
| immigration (if applicable) | **visa status timeline with exact date ranges** (mid-year changes matter: a year that leaves student status can still claim a student-article treaty benefit on income earned during the student period — eligibility is per-period, not per-year), first US entry, I-94 travel records (photo/CBP export), green-card steps |
| residency facts | days in US per year (computed from I-94 if provided), home country address |
| household | filing-status facts (marital status **on Dec 31**; HOH qualifying-person test; QSS eligibility), dependents, and — when married — the **spouse as a second taxpayer**: spouse name, SSN/ITIN, DOB, and the spouse's own immigration & residency timeline when relevant (an NRA spouse changes the available statuses and the worldwide-income question) |
| state footprint | where you **lived** (date ranges) and where you **worked** (date ranges, remote vs on-site) per tax year |
| income documents | W-2, 1099-NEC/INT/DIV/B, 1098-T, K-1 … (inventory with "have / missing / N-A"); on a joint return each document is **tagged to the taxpayer or the spouse** who earned it |
...
**Disambiguation by design.** Intake questions that users predictably get wrong ship with built-in clarification. Example: "mailing address" must be asked as *"the address where you receive mail TODAY — not where you lived during the tax year; the IRS sends bills and notices here"*, because users instinctively give their historical address. Historical addresses are collected separately under *state footprint* (they drive state scoping and Schedule C business address), never auto-copied into the return's address box. Treaty-relevant facts (visa periods, enrollment) are asked as date ranges, never as a single "what's your status" question.

**Joint vs separate, by design.** "Married" is not a filing status — a married couple chooses **married filing jointly (MFJ)** or **married filing separately (MFS)**, and that choice is a *position* with real-dollar consequences (brackets, standard deduction, credit/IRA/education phase-outs, and liability — MFJ is jointly-and-severally liable). Intake therefore (a) **determines eligibility** per status from facts, not a free-text answer (married on Dec 31 → MFJ/MFS; unmarried with a qualifying person → head of household; recent widow(er) with a dependent child → qualifying surviving spouse); (b) collects the **spouse as a full second taxpayer** (identity + the spouse's own income documents + the spouse's residency/visa facts); and (c) records the MFJ-vs-MFS decision and its dollar delta in `RECONCILIATION.md`. The **engine math is already filing-status-aware** (`calc` computes status-specific tax, standard deduction, and aged/blind additions; knowledge packs carry per-status tables) and the federal `f1040` pack maps all five status boxes plus the spouse name/SSN lines — so this is an **intake/profile/positions** addition, not an engine or form-pack change. The residency result (§4 step) **gates which statuses are even offered**: Form 1040-NR has no MFJ/HOH box by design (NRA filers are single/MFS/QSS), so for the international-student use case intake must also surface the **§6013(g)/(h) election** to treat an NRA spouse as a U.S. resident in order to file MFJ — and its trade-off, that the couple's *worldwide* income becomes taxable.
...
fields:
  - line: "identifying_number"
    ...
    maxlen: 9
    comb: true
    format: ssn_digits_only             # dashes overflow 9-cell comb fields (pitfall P-001)
...
- **Checkbox audit**: required Yes/No groups left fully `/Off` → flag (e.g. 8843 line 12, Sched OI item I — pitfall P-003).
...
`knowledge/pitfalls.yaml`: every bug found becomes (a) a permanent verifier rule, (b) a regression test, and (c) where applicable, an intake-question fix. The verify report explicitly lists each pitfall as PASS/FAIL so agents and users see the self-check happening. Seed entries:
...
| P-002 | A filer supplies a historical address, so IRS notices would go to a former residence | intake disambiguation (current-mailing vs historical); verifier warns when return address ≠ user-confirmed current address, or matches an address whose date range ended |
...
| P-004 | Risk: a treaty benefit mis-scoped on a status-change year (an F-1 → H-1B filer can still claim for student-period income) | treaty eligibility evaluated per visa **period**, encoded in `knowledge/federal/<year>.yaml` treaty rules; covered by eval scenario |

Contributors must add a pitfall entry with every bug fix (enforced in PR template).

## 11. Skill layer

`skills/claude/SKILL.md` (mirrored to `AGENTS.md` / Copilot instructions) encodes the §2 workflow with hard rules: never invent data; user confirms extracted values before filling; verify gate is mandatory; everything framed as a review draft; the user signs and files. Includes a worked example on a hypothetical nonresident student back-filing one prior year (demo numbers).
...
2. **Plain language with the math available.** Every number gets a one-sentence explanation ("you get $1,250 back because your employer withheld more than you owe"); the full line-by-line trail stays in `RECONCILIATION.md` for those who want it. Jargon (AGI, withholding, treaty) is explained on first use.
3. **User's language.** Interview and checklists in the user's language; forms are filled in English as required; `CHECKLIST.md` bilingual on request.
...
- Eval scenarios v1: (a) F-1 back-filing (1040-NR + 8843) with self-employment income, (b) simple W-2 federal+CA resident, (c) part-year CA nonresident with remote work, (d) refund + direct deposit, (e) balance due paid online vs by check, (f) no-income-tax state ("nothing to file" answer), (g) **F-1 → H-1B mid-year transition claiming a student-article treaty benefit on student-period wages**, (h) user who moved after the tax year (current vs historical address handling), (i) **post-2025 law change** (e.g., OBBBA car-loan-interest deduction on a 2026 filing): agent must resolve caps/eligibility via `get_sources` and cite IRS guidance — hallucinated numbers fail the eval, (j) **estimate accuracy & honesty**: for the simple W-2 scenario, the early estimate from W-2-only input must bracket the final computed refund and the range must tighten as intake completes; an estimate presented without its assumption list, or a point value presented as exact, fails the eval, (k) **married filing jointly**: two W-2s (taxpayer + spouse) on one return — correct MFJ standard deduction/brackets, spouse name/SSN filled, both-signature checklist, (l) **MFJ vs MFS comparison**: the server computes the return both ways and recommends the lower-tax option with the dollar delta shown; recommending without showing both, or ignoring the joint-liability caveat, fails, (m) **NRA-spouse §6013(g) election**: an F-1/H-1B couple with one nonresident-alien spouse elects to file MFJ — the worldwide-income trade-off and the election statement are surfaced and cited; silently filing MFJ for an NRA spouse without the election fails.
...
- **M3 — Intake + knowledge (2 days):** profile schema (incl. **spouse as a second taxpayer + per-document owner**), `intake_checklist` (incl. **filing-status determination + MFJ-vs-MFS comparison**, residency-gated to the statuses each filer may actually use), `estimate_refund` + roadmap output (status-aware), federal knowledge YAML (incl. tax tables/schedules as cited data), `sources.yaml` + `get_sources` + `effective_law_changes` registry, `file_and_pay` for federal (MFJ: both spouses sign).
