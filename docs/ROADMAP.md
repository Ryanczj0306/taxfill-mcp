...
Done and on `main` (**2,825 tests, all green** — offline 2,720 + live-.gov 105, exit 0;
...
- [ ] **H7 — scenario comparison surface (N-9, N-15).** Planning questions arrive
      ...
      **deltas must sum exactly to the headline number**, which is the acceptance
      criterion (in a demo what-if every itemized delta, positive or
      negative, reconciles to the headline to the dollar). It must be
      **persisted and re-runnable**
      against the workspace profile, not a one-shot call: a planning conversation
      revises facts mid-flight and each revision re-runs every scenario.
- [ ] **H8 — tax-advantaged account knowledge (N-10, N-11, N-12, N-13).** The
      ...
      * a `contribution_limits` knowledge block, cited, that records not just the
        ...
        PLAN; HSA $4,400/$8,750 per COVERAGE TIER (so two people with self-only
      ...
      * **Roth-vs-pre-tax representation** on the profile (a deferral is a split,
        ...
        phase-out.
      * an **eligibility/excess-contribution guard**: a direct
        Roth IRA contribution made by a filer whose MAGI is above the
      ...
      * a **MAGI ladder** output (gross → W-2 box 1 → AGI → each test's MAGI with
        its own threshold), since at least six different phase-out tests can fire in
        one planning question and users cannot see why their MAGI differs from their salary.
...
4. **Phase H** (planning mode, from the FIELD_NOTES gap catalogue) — **do H5→H8→H4→H7
   before H1–H3.** Rationale from [`FIELD_NOTES.md`](FIELD_NOTES.md): that
   walkthrough used 4 of the 22 tools and every number that decided the filer's
   answer has to be computed *outside* the engine, over irs.gov figures the agent
   ...
   **H5** current-year pack (first tranche DONE) → **H8** account-limit knowledge
   ...
   research pass) → **H4** projection mode + withholding/safe-harbor → **H7**
