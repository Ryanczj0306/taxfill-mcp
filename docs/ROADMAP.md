...
- [~] **G4 — FIRST TRANCHE DONE (2026-07-09): the 8 flat-rate states** (IL/PA/IN/MI/NC/CO/KY/AZ — typed StateTaxParams blocks, verifier-corrected data incl. AZ line 38-41 exemptions, `state_tax` calc op wired into Recipe C's verify-independent step). **SECOND TRANCHE (graduated states) — ENGINE DONE (2026-07-16):** `StateTaxParams` now takes per-filing-status `brackets` (exactly one of flat_rate/brackets; zero-rate floors for the OH/MS shape; contiguity enforced) and `calc.state_tax` computes marginal-bracket tax with per-bracket work lines + `rate_structure`/`marginal_rate` result fields; `base` gained `state_taxable_income` for income-dependent-deduction states (WI-style). Synthetic-schedule engine + schema tests green. **DATA IN FLIGHT:** 23 of 27 remaining states researched from official 2023 instructions (results salvaged off-repo pending verification; MO/DC/WV/DE research incomplete) — the mandatory independent verification pass was cut short by an org API spend limit; NO graduated tax block ships until each state passes it (quality bar: two-pass verification, zero discrepancies). *(was:* State returns fill/verify
