...
def _runtime_calc_ops() -> set[str]:
    ...
    assert len(ops) == 17, f"calc op count changed ({len(ops)}) — update the skills, then this number"
    return ops
...
def test_every_calc_op_is_named_in_the_full_skill() -> None:
    """`calc` is one MCP tool wrapping 17 ops — the ops are only discoverable in prose."""
