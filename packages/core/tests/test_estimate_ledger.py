...
only report two bottom lines and a delta with no attribution, and why a caller
otherwise had to rebuild the "where does the difference come from" table by
hand. Every line now carries slot/role/effect, and TWO invariants are enforced
at runtime on every computation (never only in tests):
